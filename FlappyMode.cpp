...
void FlappyMode::update(float elapsed) {
	...
	if(environ_time>10){
		environ=int((mt() / float(mt.max())) * 3.99f);
		environ_time=0;
		left_score+=1;
	}
	if(environ_time<10 && environ_time>9){
		warning=true;
	}else{
		warning=false;
	}
	...
	}else if(bars.size()>0 && bars[bars.size()-1].x<3.0f){
...
void FlappyMode::draw(glm::uvec2 const &drawable_size) {
	...
	//warning
	if(warning==true){
		draw_rectangle(glm::vec2(6.0f, 2.0f), glm::vec2(0.2f,0.5f), fg_color);	
		draw_rectangle(glm::vec2(6.0f, 1.0f), glm::vec2(0.2f,0.2f), fg_color);
	}
	//walls:
